...
JNIEXPORT jint JNICALL Java_com_mediacodec1220_mp4_ffmpegwjs_MainActivity_FFmpegCommand( JNIEnv * env, jobject thiz, jint cmdnum, jobjectArray cmdline)
{
  ...
  int bol=ffmpegmain(argc,argv);
  for(i=0;i<argc;i++){
    free(argv[i]);
  }
  free(argv);
  __android_log_print(ANDROID_LOG_ERROR, "LXS_Test", "jni_return");
  return bol;
